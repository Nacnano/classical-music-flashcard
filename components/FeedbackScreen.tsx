...
const FeedbackScreen: React.FC<FeedbackScreenProps> = ({ feedback, onNext }) => {
  ...
  return (
    <div className="w-full text-center bg-slate-800/50 p-8 rounded-xl shadow-2xl animate-fade-in">
        <div className={`w-20 h-20 rounded-full mx-auto flex items-center justify-center mb-6 ${isCorrect ? 'bg-green-500/80' : 'bg-red-500/80'}`}>
            {isCorrect ? <CheckIcon className="w-12 h-12 text-white" /> : <XIcon className="w-12 h-12 text-white" />}
        </div>
      ...
      <div className="my-6 text-lg">
        ...
        <div className="mt-4 max-w-md mx-auto">
            <YouTubePlayer youtubeId={correctPiece.youtubeId} youtubeStartTime={correctPiece.youtubeStartTime} />
        </div>
      </div>
